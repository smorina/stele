...
from stele.backends.gdsii import GdsStats, build_library, page_to_polygons, shapely_to_gdstk
...
# unique page IRs kept in memory across the plate loop: enough that a tier
# ladder re-materializing the same source pages avoids re-rendering, bounded
# so a full plate set never holds thousands of 75 MP page rasters (finding 8)
PAGE_IR_CACHE_CAP = 16
...
def build_job(manifest_path: str, verify: bool = True, preview: bool = True) -> dict:
    t0 = time.time()
...
def _build_one_plate(
    ...
    t0 = time.time()
    lib = build_library(precision=profiles.fab.dbu_nm * 1e-9)
    stats = GdsStats()
    content = lib.new_cell("CONTENT")
    cell_names = page_cell_assignments(plan)
    cell_by_key: dict[str, gdstk.Cell] = {}
    dithering = profiles.content.image_mode == "dither"
    halftone_stats: dict[str, dict] = {}
    tile_cells: dict[bytes, gdstk.Cell] = {}  # shared across pages within the plate
    if dithering:
    ...
    for pl in plan.placements:
        if pl.page_key not in cell_by_key:
            ...
            cell = lib.new_cell(cell_names[pl.page_key])
            ...
            if dithering and page_ir.images:
                ...
                patch_cell = lib.new_cell(f"{cell_names[pl.page_key]}_PATCH")
                ...
                for region in regions_to_dither:
                    ...
                    for key, bm in geo.tile_bitmaps.items():
                        if key not in tile_cells:
                            tc = lib.new_cell(f"HT_{len(tile_cells):05d}")
                ...
                cell.add(gdstk.Reference(patch_cell, (0.0, 0.0)))
                stats.references += 1
    ...
    furniture_cell = lib.new_cell("FURNITURE")
    ...
    content.add(gdstk.Reference(furniture_cell, (0.0, 0.0)))

    # --- top cell, mirroring applied as a whole-plate transform ---
    top = lib.new_cell("PLATE")
    if orientation.mirrored:
        top.add(gdstk.Reference(content, (plan.plate_w_um, 0.0), rotation=math.pi,
                                x_reflection=True))
    else:
        top.add(gdstk.Reference(content, (0.0, 0.0)))

    os.makedirs(os.path.dirname(gds_path) or ".", exist_ok=True)
    # fixed timestamp -> byte-stable output; vertex ceiling from the fab profile
    lib.write_gds(
        gds_path,
